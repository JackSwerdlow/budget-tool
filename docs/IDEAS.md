...
- **Roll Overview's pinned bar + pager out to every tab.** Overview now has the pattern: a pinned two-slot bar (sub-tabs left, month control right) over `SubTabPager` panels that each scroll themselves. The other tabs still do their own thing and look inconsistent beside it — Add has a plain `Segmented` above the old swipe detector, Salary has a non-sticky month row *above* a separate `Segmented` (`Salary.tsx:176-195`), Manage needs its structure checked. Want the same bar + pager on **all** tabs, with just the sub-tabs where a tab has no month picker. Means extracting the bar into a shared component each tab feeds its sub-tabs + optional right-slot control into. Watch: unlike Overview's read-only panels, Add/Manage mount forms with live state, so all-panels-mounted changes when their state resets.
