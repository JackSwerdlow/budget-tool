...
pub fn migrate(conn: &Connection) -> rusqlite::Result<()> {
    ...
    let _ = conn.execute(
        "ALTER TABLE salary_config ADD COLUMN untaxed_income_pence INTEGER NOT NULL DEFAULT 0",
        [],
    );
...
mod tests {
    ...
    #[test]
    fn migrate_adds_untaxed_income_column_to_older_db() {
        let c = Connection::open_in_memory().unwrap();
        c.execute_batch(
            "CREATE TABLE salary_config (year INTEGER, month INTEGER, gross_yearly_pence INTEGER, PRIMARY KEY(year,month));",
        ).unwrap();
        ...
        let cols: Vec<String> = c
            ...
            .query_map([], |r| r.get::<_, String>(1)).unwrap()
            .map(|x| x.unwrap()).collect();
        assert!(cols.iter().any(|c| c == "untaxed_income_pence"));
    }
