...
export default defineConfig(({ command }) => ({
  ...
  // origin root (http://host:8100/) or behind a sub-path reverse proxy
  // (https://code-host/proxy/8100/). Dev ignores a relative base (uses '/'), so the
  ...
  server: {
    host: '0.0.0.0',
    port: 5001,
    strictPort: true, // fail loudly if 5001 is taken rather than silently drifting ports
