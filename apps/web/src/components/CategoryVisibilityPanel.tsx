    ...
    <div className="rounded-lg border border-hairline bg-panel p-3">
      ...
      <div className="grid grid-cols-1 gap-x-6 gap-y-3 sm:grid-cols-2 md:grid-cols-3">
        {data.groups.map((group) => {
          ...
          return (
            <div key={group.id}>
              <label className="mb-1.5 flex w-fit cursor-pointer items-center gap-1.5">
                <input
                  type="checkbox"
              ...
              {/* Same M3 connected-row look as CategoryGrid, but multi-select (pressed = shown)
                 and one uniform pressed fill per group — the group colour is the middle shade
                 of its category palette. The dot keeps the per-category colour. */}
                ...
                {cats.map((c, i) => {
                  ...
                  return (
                    <button
                      ...
                      aria-pressed={shown}
                      className={`flex items-center gap-1 px-2.5 py-1 text-xs transition-all duration-100 ${roundedClass} ${
                        shown ? 'text-ink' : 'text-ink-faint hover:text-ink-muted'
                      }`}
                      style={{
                        backgroundColor: shown
                          ? `color-mix(in srgb, ${group.color} 32%, var(--color-panel))`
                          : `color-mix(in srgb, ${group.color} 8%, var(--color-panel))`,
                        boxShadow: shown ? `inset 0 0 0 1px ${group.color}` : undefined,
                      }}
                    >
                      <span
                        className={`h-2 w-2 rounded-sm ${shown ? '' : 'opacity-30'}`}
