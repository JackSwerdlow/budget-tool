...
import { createList, type NewListItemInput } from '../api';
...
export function AddList({ data }: { data: LedgerData }) {
  ...
  const [rows, setRows] = useState<Row[]>(() => [newRow(groceriesId), newRow(groceriesId)]);
  const [deliveryOpen, setDeliveryOpen] = useState(false);
  const [deliveryFeeText, setDeliveryFeeText] = useState('');
  const [deliverySharePct, setDeliverySharePct] = useState(0);
  const [deliveryCategoryId, setDeliveryCategoryId] = useState(groceriesId);
  ...
  async function onSave() {
    ...
    try {
      ...
      await refresh();
      setRows([newRow(groceriesId), newRow(groceriesId)]);
      setNote('');
      setDeliveryOpen(false);
      setDeliveryFeeText('');
      setDeliverySharePct(0);
      setSaved(`Saved — ${formatGBP(savedMine)} filed to your share.`);
    } catch (err) {
      setError(String(err));
    } finally {
      setSubmitting(false);
