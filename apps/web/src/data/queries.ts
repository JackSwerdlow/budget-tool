...
type SalaryConfigRow = Omit<SalaryConfig, 'sl_enabled'> & { sl_enabled: number; bonus_pence: number };
    ...
    async saveSalaryConfig(cfg, netMonthlyPence) {
      await exec.execute(
        `INSERT INTO salary_config (
           ...
           ni_lower_monthly_pence, ni_upper_monthly_pence, ni_primary_pct, ni_upper_pct,
           sl_enabled, sl_threshold_yearly_pence, sl_rate_pct,
           sl_balance_pence, sl_interest_rate_pct, bonus_pence
         ) VALUES (
        ...
        [
          ...
          cfg.sl_balance_pence ?? null, cfg.sl_interest_rate_pct ?? null, cfg.bonus_pence ?? 0,
        ],
      );
